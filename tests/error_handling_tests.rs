...
fn test_genai_error_display() {
    ...
    assert_eq!(parse_error.to_string(), "SSE parsing error: test parse error");
    
    let api_error = GenaiError::Api("API error message".to_string());
    assert_eq!(api_error.to_string(), "API Error returned by Google: API error message");
    
    let internal_error = GenaiError::Internal("internal error".to_string());
    assert_eq!(internal_error.to_string(), "Internal client error: internal error");
}
