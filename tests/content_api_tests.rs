use rust_genai::{user_text, model_function_call, user_tool_response, build_content_request, FunctionDeclaration};
use serde_json::json;
...
fn test_build_content_request_with_tools() {
    ...
    let conversation = vec![
        user_text("What's the weather in Paris?".to_string()),
    ];
...
fn test_content_api_edge_cases() {
    ...
    assert_eq!(content.parts[0].function_call.as_ref().unwrap().args, empty_args);
} 