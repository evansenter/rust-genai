...
fn test_basic_function_declaration() {
    ...
    let decl = test_basic_declaration();
...
fn test_function_with_doc_comment() {
    ...
    let decl = test_with_docs_declaration();
    assert_eq!(decl.name, "test_with_docs");
    assert_eq!(decl.description, "This is a test function that does something");
}
...
fn test_with_param_metadata() {
    ...
    let decl = greet_person_declaration();
...
fn test_optional_parameters() {
    ...
    let decl = test_optional_declaration();
...
fn test_enum_values() {
    ...
    let decl = convert_temp_declaration();
    let params_json = serde_json::to_string(&decl.parameters).unwrap();
...
fn test_various_types() {
    ...
    let decl = test_types_declaration();
    assert_eq!(decl.required, vec!["text", "count", "amount", "flag", "items", "data"]);
...
fn test_multiline_doc_comment() {
    ...
    let decl = test_multiline_declaration();
    assert_eq!(decl.description, "This is a function that\ndoes multiple things:\n- First thing\n- Second thing");
}
...
fn test_param_without_metadata_no_description() {
    ...
    let decl = test_no_param_desc_declaration();
    let params_json = serde_json::to_string(&decl.parameters).unwrap();
...
fn test_param_docs_not_allowed() {
    ...
    */
    ...
    #[generate_function_declaration(
        name(description = "This is the correct way to add param descriptions")
    )]
    fn test_correct_param_desc(name: String) -> String {
        name
    }
    
    let decl = test_correct_param_desc_declaration();
    let params_json = serde_json::to_string(&decl.parameters).unwrap();
